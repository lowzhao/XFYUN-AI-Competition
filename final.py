...
lost = 0
total = 0;
def createdata(filename, destination,total,lost, **kwargs):
	duration = kwargs['dur']
...
def load_data(testsplit=0.2, balance=True, size=1000):
    ...
    for dialect in dialectArray:
        ...
        #         for fn in files:
            ...
            Xi.append(xs)
            yi = list((np.ones(len(Xi))*1).astype(int))
...
def ConvJS(X_train, X_test, y_train, y_test):
    ...
    model.add(Convolution2D(64,(3,3),
                            border_mode='valid', activation='relu', init='glorot_normal'))
    model.add(ZeroPadding2D((1, 1)))
    ...
    model.compile(loss='categorical_crossentropy', optimizer='sgd')
    model.fit(X_train, y_train, batch_size=2, nb_epoch=15,
              verbose=1, validation_split=0.2)
    y_pred = model.predict_classes(X_test)
    print("Multipayer 2d-conv net Result")
    # print(y_test)
...
# for dialect in dialectArray:
        # for data in dialect[data]:
dialect = "hebei"
for index in xrange(10, 31):
	if(index < 10):
		...
		createdata(dialect+"/train/trimmed/speaker0"+str(index), destination,
					dur=1, discard_short=False, fft_win=30, fft_hop=25, n_mfcc=13)
	else:
		...
		total, lost =createdata(dialect+"/train/trimmed/speaker"+str(index), destination,total,lost,
					dur=1, discard_short=False, fft_win=30, fft_hop=25, n_mfcc=13)
